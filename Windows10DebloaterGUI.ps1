...
$global:Bloatware = @(
    ...
    "Microsoft.People"
    "Microsoft.Print3D"
    "Microsoft.Paint3D"                                 # Deprecated Nov 2024
    ...
    "Microsoft.ZuneMusic"
    "Microsoft.ZuneVideo"
    "Microsoft.MixedReality.Portal"                     # Deprecated/Discontinued
    ...
    #Modern bloatware (added in newer Windows 10 builds)
    "Clipchamp.Clipchamp"
    "Microsoft.Todos"
...
$global:WhiteListedApps = @(
    ...
    "Microsoft.ScreenSketch"                    # Issue 55: Snipping Tool / Snip & Sketch
    ...
    "Slack"                                     # Issue 83
    "*Nvidia*"                                  # Issue 198
    "*Acer*"                                     # OEM apps protected
    "*Asus*"                                     # OEM apps protected
    "*Dell*"                                     # OEM apps protected
    "*HP*"                                       # OEM apps protected
    "*Lenovo*"                                   # OEM apps protected
    "*Toshiba*"                                  # OEM apps protected
)
...
$DisableWindowsUpdate.width      = 225
$DisableWindowsUpdate.height     = 30
$DisableWindowsUpdate.Anchor     = 'top,right,left'
$DisableWindowsUpdate.location   = New-Object System.Drawing.Point(10,200)
$DisableWindowsUpdate.Font       = New-Object System.Drawing.Font('Consolas',9)
$DisableWindowsUpdate.ForeColor  = [System.Drawing.ColorTranslator]::FromHtml("#eeeeee")

$EnableWindowsUpdate             = New-Object system.Windows.Forms.Button
$EnableWindowsUpdate.FlatStyle   = 'Flat'
$EnableWindowsUpdate.text        = "ENABLE WINDOWS UPDATE"
$EnableWindowsUpdate.width       = 225
$EnableWindowsUpdate.height      = 30
$EnableWindowsUpdate.Anchor      = 'top,right,left'
$EnableWindowsUpdate.location    = New-Object System.Drawing.Point(245,200)
$EnableWindowsUpdate.Font        = New-Object System.Drawing.Font('Consolas',9)
$EnableWindowsUpdate.ForeColor   = [System.Drawing.ColorTranslator]::FromHtml("#eeeeee")
...
$OtherPanel.controls.AddRange(@($Other,$RemoveOnedrive,$InstallNet35,$DisableWindowsUpdate,$EnableWindowsUpdate,$UnpinStartMenuTiles,$DisableTelemetry,$RemoveRegkeys))
...
Write-Output "Running safety checks and creating System Restore Point. Please wait..."
If (Get-Command Run-SafetyChecks -ErrorAction SilentlyContinue) {
    Try {
        Run-SafetyChecks -Description "Before using W10DebloaterGUI.ps1"
    } Catch {
        Log-Error -Message "Safety checks failed: $($_.Exception.Message)" -Source "Windows10DebloaterGUI.ps1"
        Exit
    }
}
...
$CustomizeBlacklist.Add_Click( {
        ...
        $SelectAll.width                = 275
        $SelectAll.height               = 30
        $SelectAll.Location             = New-Object System.Drawing.Point(10, 530)
        $SelectAll.Font                 = New-Object System.Drawing.Font('Consolas',9)
        $SelectAll.ForeColor            = [System.Drawing.ColorTranslator]::FromHtml("#eeeeee")

        $DeselectAll                    = New-Object System.Windows.Forms.Button
        $DeselectAll.FlatStyle          = 'Flat'
        $DeselectAll.Text               = "Deselect All"
        $DeselectAll.width              = 275
        $DeselectAll.height             = 30
        $DeselectAll.Location           = New-Object System.Drawing.Point(295, 530)
        $DeselectAll.Font               = New-Object System.Drawing.Font('Consolas',9)
        $DeselectAll.ForeColor          = [System.Drawing.ColorTranslator]::FromHtml("#eeeeee")
        ...
        $SaveList.width                 = 560
        $SaveList.height                = 30
        $SaveList.Location              = New-Object System.Drawing.Point(10, 570)
        $SaveList.Font                  = New-Object System.Drawing.Font('Consolas',9)
        $SaveList.ForeColor             = [System.Drawing.ColorTranslator]::FromHtml("#eeeeee")

        $CustomizeForm.controls.AddRange(@($SaveList,$SelectAll, $DeselectAll, $ListPanel))

        $SelectAll.Add_Click( {
            @($ListPanel.controls) | ForEach {
                if ($_ -is [System.Windows.Forms.CheckBox] -and $_.Enabled) {
                    $_.Checked = $true
                }
            }
        })
        ...
        $SaveList.Add_Click( {
               # $ErrorActionPreference = 'SilentlyContinue'
                ...
                # Escape each app name to prevent dots and special characters from causing partial matches
                $global:BloatwareRegex = ($global:Bloatware | ForEach-Object { [regex]::Escape($_) }) -join '|'
                $global:WhiteListedAppsRegex = ($global:WhiteListedApps | ForEach-Object { [regex]::Escape($_) }) -join '|'
            })

        Function FixWhitelistedApps {
            $WhitelistedAppsToFix = @(
            ...
            ForEach ($App in $WhitelistedAppsToFix) {
                # Check if it's missing or needs re-registering
                $Pkg = Get-AppxPackage -AllUsers -Name $App
                If ($Pkg) {
                    $Pkg | ForEach { Add-AppxPackage -DisableDevelopmentMode -Register "$($_.InstallLocation)\AppXManifest.xml" -ErrorAction SilentlyContinue }
                }
            }
        }

        $CustomizeForm.ClientSize       = New-Object System.Drawing.Point(580,610)
        ...
        Function AddAppToCustomizeForm() {
            ...
            $Checkbox.Location = New-Object System.Drawing.Point(280, (0 + $position * 25))
            $Checkbox.Autosize = 1;
        ...
        ForEach ( $item in $AllUsers ) {
            ...
            if ( $null -notmatch $Online -and $Online -cmatch $item) { $string += " Online" }
            # Changed to default to $false (unchecked) to protect user-installed store apps
            AddAppToCustomizeForm $checkboxCounter $item $true $false $true $string
            ++$checkboxCounter
        }
...
$RemoveBlacklistedBloatware.Add_Click( { 
        ...
        Function DebloatBlacklist {
            ...
            # Use safety filters even for custom lists to protect whitelisted apps
            Get-AppxPackage | Where-Object { ($_.Name -cmatch $global:BloatwareRegex) -and !($_.Name -cmatch $global:WhiteListedAppsRegex) -and !($NonRemovables -cmatch $_.Name) } | Remove-AppxPackage
            Write-Host "...now starting the silent ProvisionedPackage removal..."
            Get-AppxProvisionedPackage -Online | Where-Object { ($_.DisplayName -cmatch $global:BloatwareRegex) -and !($_.DisplayName -cmatch $global:WhiteListedAppsRegex) -and !($NonRemovables -cmatch $_.DisplayName) } | Remove-AppxProvisionedPackage -Online
            Write-Host "...and the final cleanup..."
            Get-AppxPackage -AllUsers | Where-Object { ($_.Name -cmatch $global:BloatwareRegex) -and !($_.Name -cmatch $global:WhiteListedAppsRegex) -and !($NonRemovables -cmatch $_.Name) } | Remove-AppxPackage
        }
...
$RemoveAllBloatware.Add_Click( { 
        ...
        Function Protect-Privacy { 
            ...
            #Turns off Data Collection via the AllowTelemetry key by changing it to 0
            ...
            If (!(Test-Path $BingSearch)) {
                New-Item $BingSearch -ErrorAction SilentlyContinue
            }
        ...
        Function UnpinStart {
            ...
            # Modern approach: Clearing the CloudStore registry key efficiently wipes the Start Menu tile layout
            $CloudStore = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\CloudStore'
            If (Test-Path $CloudStore) {
                Write-Host "Stopping Explorer to clear CloudStore cache"
                Stop-Process -Name explorer -Force
                Start-Sleep -Seconds 2
                Remove-Item $CloudStore -Recurse -Force
                Start-Process explorer.exe
                Write-Host "Start menu tiles have been successfully unpinned"
            } else {
                Write-Host "CloudStore registry key not found, start menu may already be unpinned"
            }
        }
...
$DisableTelemetry.Add_Click( { 
        ...
        #BingSearchEnabled is ignored on 22H2+, use DisableSearchBoxSuggestions instead
        $SearchExplorer = "HKCU:\SOFTWARE\Policies\Microsoft\Windows\Explorer"
        If (!(Test-Path $SearchExplorer)) {
            New-Item $SearchExplorer
        }
        Set-ItemProperty $SearchExplorer DisableSearchBoxSuggestions -Value 1
        #Also set BingSearchEnabled for older builds
        $SearchReg = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search"
        If (Test-Path $SearchReg) {
            Set-ItemProperty $SearchReg BingSearchEnabled -Value 0
        }
        ...
        #Wi-Fi Sense was removed in Windows 10 version 1607 (Anniversary Update)
        #The AllowWiFiHotSpotReporting and AllowAutoConnectToWiFiSenseHotspots registry keys are obsolete
        Write-Host "Skipping Wi-Fi Sense (removed in version 1607)"
        ...
        Write-Host "Disabling Application Experience telemetry tasks"
        Get-ScheduledTask "Microsoft Compatibility Appraiser" -ErrorAction SilentlyContinue | Disable-ScheduledTask
        Get-ScheduledTask "ProgramDataUpdater" -ErrorAction SilentlyContinue | Disable-ScheduledTask
        Get-ScheduledTask "Proxy" -TaskPath "\Microsoft\Windows\Autochk\" -ErrorAction SilentlyContinue | Disable-ScheduledTask
...
$DisableWindowsUpdate.Add_Click( {
        ...
        Write-Host "Windows Update services disabled."
    } )
...
$EnableWindowsUpdate.Add_Click( {
        ...
        Write-Host "Windows Update services restored."
    } )
