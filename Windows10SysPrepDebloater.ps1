...
param (
  [switch]$Debloat, [switch]$SysPrep, [switch]$Privacy
)
...
Function Protect-Privacy {
    ...
    #Disables Web Search in Start Menu - compatible with 22H2
    Write-Output "Disabling Bing Search in Start Menu"
    If (!(Test-Path $Search)) {
        New-Item $Search -ErrorAction SilentlyContinue
    }
    ...
    If (!(Test-Path $SearchExplorer)) {
        New-Item $SearchExplorer -ErrorAction SilentlyContinue
    }
    Set-ItemProperty $SearchExplorer DisableSearchBoxSuggestions -Value 1 -ErrorAction SilentlyContinue
    ...
    If (Test-Path $DataCollection1) {
        Set-ItemProperty $DataCollection1 -Name AllowTelemetry -Value 0 -Verbose
    }
    If (!(Test-Path $DataCollection2)) {
        New-Item $DataCollection2 -ErrorAction SilentlyContinue
    }
    Set-ItemProperty $DataCollection2 -Name AllowTelemetry -Value 0 -Verbose -ErrorAction SilentlyContinue
    ...
    Get-ScheduledTask -TaskName "Microsoft Compatibility Appraiser" -ErrorAction SilentlyContinue | Disable-ScheduledTask
    Get-ScheduledTask -TaskName "ProgramDataUpdater" -ErrorAction SilentlyContinue | Disable-ScheduledTask
    Get-ScheduledTask "Proxy" -TaskPath "\Microsoft\Windows\Autochk\" -ErrorAction SilentlyContinue | Disable-ScheduledTask

    #Stopping and disabling Diagnostics Tracking Service (Connected User Experiences and Telemetry)
...
#If no switches are passed, run everything for backward compatibility
If (!$SysPrep -and !$Debloat -and !$Privacy) {
    $SysPrep = $true
    $Debloat = $true
    $Privacy = $true
}
...
If ($Privacy) {
    Write-Output "Stopping telemetry, disabling unnecessary scheduled tasks, and preventing bloatware from returning."
    Protect-Privacy
}
